/**
 * useAnalytics
 * Buffers listen events locally and flushes to the ingest-events Edge Function
 * every FLUSH_INTERVAL ms, or immediately on complete/unmount.
 */
  ...
  const flush = useCallback(async (force = false) => {
    ...
    try {
      await fetch(EDGE_FN_URL, {
        ...
        keepalive: force, // keepalive on unload so beacon survives tab close
      });
    } catch {
      // On failure, put events back so next flush picks them up
