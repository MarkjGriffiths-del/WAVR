...
interface Project {
  id: string; title: string; updated_at: string; versions: Version[];
}
interface Version {
  id: string; version_number: number; label: string | null;
  duration_ms: number; is_spatial: boolean; status: string;
  waveform_peaks: number[] | null; storage_path: string; uploaded_at: string;
}
...
export function Dashboard() {
  ...
  const openPlayerRef    = useRef<((p: Project, auto?: boolean) => void) | null>(null);
  const audioControlsRef = useRef<{ seekTo: (ms: number) => void; play: () => void } | null>(null);
  ...
  // onEnded uses refs so it never stales
  const handleEnded = useCallback(() => {
    const ap       = activeProjectRef.current;
    const projs    = projectsRef.current;
    const r1       = repeatOneRef.current;
    const rAll     = repeatAllRef.current;
    const controls = audioControlsRef.current;
    const opener   = openPlayerRef.current;
    if (!ap || !controls || !opener) return;
    if (r1) {
      controls.seekTo(0);
      setTimeout(() => controls.play(), 100);
    } else {
      const i    = projs.findIndex(p => p.id === ap.id);
      const next = rAll ? projs[(i + 1) % projs.length] : projs[i + 1];
      if (next) opener(next, true);
    }
  }, []);
  ...
  const createShareLink = async () => {
    ...
    await supabase.from('share_links').insert({
      project_id: activeProject.id, slug,
      allow_comments: false, allow_download: false,
    });
    setShareSlug(slug);
  };
  ...
  const goBack = () => { setView('list'); setMenuOpen(false); };
    ...
    return (
      <div style={{ ...s.page, position: 'relative', overflow: 'hidden', minHeight: '100vh', background: '#000' }}>
        ...
        <div style={{ position: 'relative', zIndex: 2, opacity: uiOpacity, transition: 'opacity 0.5s ease', pointerEvents: uiOpacity < 0.1 ? 'none' : 'auto' }}>
          <div style={s.topbar}>
            <button style={s.backBtn} onClick={goBack}><i className="ti ti-chevron-left" /> Back</button>
            <div style={s.topbarTitle}>Now playing</div>
            <button style={s.menuBtn} onClick={() => setMenuOpen(m => !m)}><i className="ti ti-dots" /></button>
          </div>
          {menuOpen && (
            <div style={s.dropdown}>
              {shareSlug
                ? <button style={s.dropItem} onClick={() => { copyLink(); setMenuOpen(false); }}>{copied ? '✓ Copied!' : '🔗 Copy share link'}</button>
                : <button style={s.dropItem} onClick={() => { createShareLink(); setMenuOpen(false); }}>🔗 Generate share link</button>}
          ...
          <div style={s.trackInfo}>
            ...
            <div style={s.trackSub}>
              {activeVersion.is_spatial && <span style={{ color: '#a78bfa' }}>● Spatial · </span>}
        ...
        {/* Share + screensaver — fades out */}
        <div style={{ position: 'relative', zIndex: 2, opacity: uiOpacity, transition: 'opacity 0.5s ease', pointerEvents: uiOpacity < 0.1 ? 'none' : 'auto' }}>
          <div style={s.shareStrip}>
            {shareSlug
              ? <button style={s.shareBtn} onClick={copyLink}><i className="ti ti-link" /> {copied ? 'Link copied!' : 'Copy share link'}</button>
              : <button style={s.shareBtn} onClick={createShareLink}><i className="ti ti-share" /> Generate share link</button>}
          </div>
          <div style={{ display: 'flex', justifyContent: 'center', paddingBottom: 8 }}>
            <button style={s.ssBtn} onClick={ssTrigger}><i className="ti ti-moon" /> Screensaver</button>
          </div>
        </div>
    ...
    <div style={s.page}>
      ...
      <div style={s.list}>
        ...
        {projects.map(p => {
          ...
          return (
            <div key={p.id} style={{ ...s.trackRow, ...(isActive ? s.trackRowActive : {}) }} onClick={() => openPlayer(p)}>
